...
def max_pool_1x1_d3(x):
    return tf.nn.pool(x, window_shape=[3],
                            pooling_type="MAX", padding='SAME', strides=[3])
...
h_pool3 = max_pool_1x1_d3(h_conv3)


# Densely Connected Layer with 1024 neurons: (300/2/2 = 75 / 5 = 15)
W_fc1 = weight_variable([25 * 128, 64*64])
b_fc1 = bias_variable([64*64])

h_pool3_flat = tf.reshape(h_pool3, [-1, 25 * 128])
...
with tf.Session() as sess:
    ...
    myFile = open('3_Layer_MAX_relu_batch200.csv','w')
    with myFile:
